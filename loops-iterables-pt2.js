...
var contains = function(myArr,inputString) {
    // log('is ' + inputString + ' in the array?')
...
var nicer = function(inputstring) {
    //local variables
 	var swearjar = ["crappy", "heck", "darn", "dang"] 
    ...
    for(var i=0; i < wordsarray.length; i++) {
        ...
        if( !contains(swearjar,word)) {
           cleanarray.push(word)
            }
            }
    var outputstring = cleanarray.join(" ")
    return outputstring
...
var capitalizeAll = function(inputstring) {
   var stringarray = inputstring.split(" ")
    ...
    for(var i = 0; i < stringarray.length; i ++) { 
       ...
       word = word[0].toUpperCase() + restofword
